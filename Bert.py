...
def self_attention(X):
    ...
    V = X@W_V
    K = np.transpose(K)
    X1 = (Q@K@V)/X_Position_Embedding.shape[2]
    X1 = Dense(24, activation='softmax')(X)
    print(X1.shape)
    return X
...
output = Dense(24,activation='relu')(X_transformer1)
...
model.fit(X_train, Y_train, epochs=300, batch_size=batch)
